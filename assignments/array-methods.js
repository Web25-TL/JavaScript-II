...
console.log("1) Full name array:", fullName);
...
console.log("2) SRY CAPS:", allCaps);
...
console.log("3) Runners who need to pick another size:", largeShirts);
...
console.log(`4) Total amount of donations is $${ticketPriceTotal}.`);
...
console.log(
  "Array methods strech 1)",
  `We need to order: ${smallShirts.length} small shirts, ${mediumShirts.length} medium shirts, ${largeShirts.length} large shirts, ${xlShirts.length} XL shirts, ${twoXlShirts.length} 2XL shirts, and ${threeXlShirts.length} 3XL shirts.`
);
...
runners.forEach((arr, index) => {
  ...
  emailList.push(arr.email);
});

console.log("Array methods strech 2) Email list:", emailList);
...
runners.forEach((arr, i) => {
  if (arr.company_name == "Skinix") {
    skinixRunners.push(
      `${arr.first_name} ${arr.last_name} is with the company, Skinix`
    );
  }
});

console.log("Array methods strech 3) Skinix runners:", skinixRunners);
